...
**Then** a new file `Untitled.md` is created in `~/MarkdownNotes/`
...
1. Note the count of files in `~/MarkdownNotes/` (e.g. `ls ~/MarkdownNotes/ | wc -l`).
2. Click the **+** button in the sidebar header.
3. Confirm `~/MarkdownNotes/Untitled.md` now exists (or `Untitled 1.md` etc. if "Untitled" was already taken).
