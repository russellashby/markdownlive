...
As a user, I want notes I delete from `~/MarkdownNotes/` outside the app to disappear from the sidebar so that the panel always reflects what's actually on disk.

## Acceptance criteria

**Given** the app is open and a note "scratch" exists in the sidebar
**When** the underlying `~/MarkdownNotes/scratch.md` is deleted from outside the app
