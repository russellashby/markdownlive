...
As a user, I want files I drop into `"$PROJECT"/` from outside the app (Finder, terminal, another tool) to show up immediately so that I never have to restart to see them.

## Acceptance criteria

**Given** the app is open
**When** a new `.md` file is created in `"$PROJECT"/` from outside the app
...
5. Repeat with a copy via Finder (drag a `.md` file from elsewhere into the `"$PROJECT"/` folder using Finder, not the app window) and confirm the same behaviour.
