...
**Then** the underlying file in `~/MarkdownNotes/` is renamed to match the new title
...
2. Confirm `~/MarkdownNotes/Draft One.md` exists.
3. Click into the title field and change it to **Final**.
4. Wait one second.
5. Confirm `~/MarkdownNotes/Final.md` exists and `Draft One.md` no longer exists.
